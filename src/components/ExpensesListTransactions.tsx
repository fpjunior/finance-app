...
export default function ExpensesListTransactions() {
  const { data } = useStoreTransaction();
  const { totalExpenses } = useTransactionContext();
  const listExpenses = data.filter(
    (item) => item.transactionType === "Expenses"
  );

  let indiceComa = listExpenses[0]?.date.indexOf(",");
  let newDate = listExpenses[0]?.date.substring(indiceComa + 1);

  let indiceComa2 = listExpenses[listExpenses.length - 1]?.date.indexOf(",");
  let newDate2 = listExpenses[listExpenses.length - 1]?.date.substring(
    indiceComa2 + 1
  );
